...
int strip_ext(char *buf, int buf_size, const char * name) {
    int size;
    char * pos;

    size = snprintf(buf,buf_size,"%s", name);
    CHECK_ERRNO(size >= buf_size, "buffer name overflow");
...
int strip_filename(char *buf, int buf_size, const char * name) {
    int size;
    char * pos;
    size = snprintf(buf,buf_size,"%s", name);
    CHECK_ERRNO(size >= buf_size, "buffer name overflow");

    pos = strrchr(buf,DIRSEP);
    if (!pos) {
        buf[0] = '\0';
        return -1;
    }
...
int get_streamfile_size(FILE * streamFile) {
    size_t current, size = 0;

    current = ftello(streamFile);
    fseeko(streamFile,0,SEEK_END);
    size = ftello(streamFile);
    fseeko(streamFile,current,SEEK_SET);
